...
namespace P011_IsTakip.WebUI.Controllers.ModelsController
{
    public class TedarikciController : Controller
    {
        ...
        {
            var model = await _tedarikciService.GetListAsync();
            return View(model);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync(Tedarikci model)
        {
            if (model == null)
            {
            ...
            model.Aktif = true;
            model.Silindi = false;
            await _tedarikciService.AddAsync(model);
        ...
        public IActionResult Edit(int id)
        {
            ...
            var model = _tedarikciService.GetById(id);
        ...
        [HttpPost]
        public IActionResult Edit(Tedarikci model)
        {
            if (model == null)
            {
                return RedirectToAction(nameof(IndexAsync));
            ...
            var data = _tedarikciService.GetById(model.Id);
        ...
        public IActionResult Detail(int id)
        {
            ...
            var data = _tedarikciService.GetById(id);
            if (data is null)
                return RedirectToAction(nameof(IndexAsync));
        ...
        [HttpPost]
        public IActionResult Delete(Tedarikci model)
        {
            if (model is null)
                return RedirectToAction(nameof(IndexAsync));
